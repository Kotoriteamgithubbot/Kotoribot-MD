...
const loadplugin = async(client, m, command, q) => {
    ...
    let code = m.quoted.text + "let file = require.resolve(__filename) fs.watchFile(file, () => { fs.unwatchFile(file) console.log(chalk.redBright(`Update'${__filename}'`)) delete require.cache[file] require(file)})"
    let path = q
